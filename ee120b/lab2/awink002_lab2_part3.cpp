...
Exercise Description: 7seg and led counter
I acknowledge all content contained herein, excluding template or example code, is my own original work.
Demo Link: https://youtu.be/0IWRWBer_jg
*/
...
void outNum(int num) {
    PORTB = (nums[num] & 0x1F);
    PORTD = ((nums[num] & 0x60) << 1) | (num << 2);
}

unsigned int num = 0;
enum states {hold, inc, dec, wait} state; //sm enum
...
void Tick() {
    ...
    switch(state) {
        case hold:
            if ((PINC & 0x01)) {state = dec;}
            else if ((PINC & 0x02)) {state = inc;}
            else {state = hold;}
            break;
        case wait:
